...
da_osi450 = xr.open_dataarray(
    "https://thredds.met.no/thredds/dodsC/osisaf/met.no/reprocessed/ice/conc_v2p0_nh_agg?xc[0:1:431],yc[0:1:431],lat[0:1:431][0:1:431],lon[0:1:431][0:1:431],time[0:1:11800],ice_conc[0:1:11800][0:1:431][0:1:431]"
)
...
da_osi430b = xr.open_dataarray(
    "https://thredds.met.no/thredds/dodsC/osisaf/met.no/reprocessed/ice/conc_crb_nh_agg?xc[0:1:431],yc[0:1:431],lat[0:1:431][0:1:431],lon[0:1:431][0:1:431],time[0:1:2008],ice_conc[0:1:2008][0:1:431][0:1:431]"
)
...
### Preprocess the data
da /= 100.0  # Convert from SIC % to fraction
...
# Load grid cell mask
mask_dict = {
    month: np.load(mask_fpath_format.format("{:02d}".format(month)))
...
for date in tqdm(dates):
    ...
    da.loc[date].data[~mask] = 0.0

    # Grab polar hole
    skip_interp = False
    if date <= config.polarhole1_final_date:
        polarhole_mask = np.load(
            os.path.join(config.mask_data_folder, config.polarhole1_fname)
        )
    elif date <= config.polarhole2_final_date:
        polarhole_mask = np.load(
            os.path.join(config.mask_data_folder, config.polarhole2_fname)
        )
    elif date <= config.polarhole3_final_date and config.use_polarhole3:
        polarhole_mask = np.load(
            os.path.join(config.mask_data_folder, config.polarhole3_fname)
        )
    else:
        skip_interp = True
