...
save_active_grid_cell_masks = (
    True  # Save the monthly land-lake-ocean masks for active gridcells
)
save_land_mask = True  # Save the land mask (constant across months)
save_arctic_region_mask = (
    True  # Save Arctic region mask from NSIDC, with coastline cells
)
...
retrieve_cmd_template_osi450 = (
    "wget --quiet -m -nH --cut-dirs=4 -P "
    + temp_ice_data_folder
...
for month in range(1, 13):
    ...
    if len(os.listdir(month_path)) > 1:
        ...
        with xr.open_dataset(day_path) as ds:
            ...
            max_extent_mask = (
                np.sum(binary[:, :, [7, 6, 0]], axis=2).reshape(432, 432) >= 1
            )
...
if save_arctic_region_mask:
    ...
    retrieve_arctic_region_mask_cmd = (
        "wget --quiet -m -nH --cut-dirs=6 -P {} "
        "ftp://sidads.colorado.edu/pub/DATASETS/seaice/polar-stereo/tools/region_n.msk"
    )
    os.system(
        retrieve_arctic_region_mask_cmd.format(os.path.join(config.mask_data_folder))
    )

    # Download SIC data for regridding
    sic_day_fpath = os.path.join(
        config.mask_data_folder, "ice_conc_nh_ease2-250_cdr-v2p0_197901021200.nc"
    )
    ...
    regions = iris.cube.Cube(
        region_data, dim_coords_and_dims=[(y_coord, 0), (x_coord, 1)]
    )

    regions_ease = regions.regrid(
        sic_EASE_cube, iris.analysis.Nearest(extrapolation_mode="mask")
    )
    ...
    sea_mask = np.ma.masked_array(np.full((432, 432), 0.0))
    ...
    np.save(
        os.path.join(config.mask_data_folder, config.region_mask_filename),
        arctic_region_mask,
    )
    ...
    if save_figures:
        ...
        im = axes[0].imshow(
            regions_ease.data.data, cmap=plt.get_cmap("tab20c", 14), clim=(-0.5, 13.5)
        )
        ...
        region_names = [
            ...
            "Bering Sea",
            "Hudson Bay",
            "Baffin Bay - Davis Strait - Labrador Sea",
            "Greenland Sea",
        ...
        axes[1].contour(
            regions_ease.data.data, levels=np.arange(0, 13, 1), colors="r", linewidths=2
        )
