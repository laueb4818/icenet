...
if __name__ == "__main__":
    ...
    if np.sum([compute_in_memory, compute_with_dask]) != 1:
        print(compute_in_memory)
        raise ValueError(
            "You must specify a single compute strategy "
    ...
    multi_index = utils.create_results_dataset_index(
        model_compute_list, leadtimes, all_target_dates, icenet_ID
    )
    results_df = pd.DataFrame(
        index=multi_index, columns=metric_compute_list, dtype=np.float32
    )
    results_df = results_df.sort_index()

    compute_dict = {model: metric_compute_list for model in model_compute_list}
    ...
    for model, model_metric_compute_list in compute_dict.items():
        ...
        if icenet_ID_match:
            sip_da = (
                heldout_forecasts_dict[model]
                .sel(ice_class=["marginal_ice", "full_ice"])
                .sum("ice_class")
            )
        ...
        for metric in model_metric_compute_list:
            if metric == "Binary accuracy":
                binary_correct_da = (binary_forecast_da == binary_true_da).astype(
                    np.float32
                )
                ...
                binary_forecast_weighted_da = binary_forecast_da.astype(int).weighted(
                    mask_da
                )
