...
biascorrection_dates = pd.date_range(start="2002-01-01", end="2011-12-01", freq="MS")
...
# Raw SEAS5 forecasts
fpaths = sorted(
    [os.path.join(seas5_EASE_folder, f) for f in os.listdir(seas5_EASE_folder)]
)
...
biascorrected_da.data[biascorrected_da.data >= 1.0] = 1.0
biascorrected_da.data[biascorrected_da.data <= 0.0] = 0.0
...
# Compute SIP
sip_da = (biascorrected_da > 0.15).sum("number") / seas5_forecast_da.number.size
