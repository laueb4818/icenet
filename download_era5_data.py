...
import os
import numpy as np
from utils import assignLatLonCoordSystem, fix_near_real_time_era5_func, xarray_to_video
...
def regrid_var(variable, EASE_path):
    ...
    print(
        "\nRegridding and saving {} reanalysis data... ".format(variable),
        end="",
        flush=True,
    )
    tic = time.time()
    ...
    if variable in ["tos", "zg500", "zg250"]:
        if variable == "tos":
            ...
            cube_ease.data[cube_ease.data > 500.0] = 0.0
            cube_ease.data[cube_ease.data < 0.0] = 0.0

            cube_ease.data[:, land_mask] = 0.0
...
if variable != "rsds_and_rsus":
    ...
    for radiation_variable in ("rss", "rsds"):
        ...
        latlon_path = os.path.join(
            config.obs_data_folder, "{}_latlon.nc".format(radiation_variable)
        )

        retrieve_CDS_data(
            var_dict["{}_cdi_name".format(radiation_variable)], latlon_path
        )
    ...
    for radiation_variable in ("rsus", "rsds"):
        # Regrid to EASE
        EASE_path = os.path.join(
            config.obs_data_folder, "{}_EASE.nc".format(radiation_variable)
        )
