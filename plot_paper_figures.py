...
ice_edge_region_df_fpath = os.path.join(
    config.uncertainty_results_folder, "ice_edge_region_results.csv"
)
uncertainty_df_fpath = os.path.join(
    config.uncertainty_results_folder, "uncertainty_results.csv"
)
sip_bounding_df_fpath = os.path.join(
    config.uncertainty_results_folder, "sip_bounding_results.csv"
)
...
ice_edge_region_df["Forecast date"] = pd.to_datetime(
    ice_edge_region_df["Forecast date"]
)
ice_edge_region_df = ice_edge_region_df.set_index(["Leadtime", "Forecast date"])

### Permute-and-predict results
pap_results_df_fpath = os.path.join(
    config.permute_and_predict_results_folder, "permute_and_predict_results.csv"
)
...
sie_errors_df = pd.read_csv(
    os.path.join(config.data_folder, "sea_ice_outlook_errors.csv"), comment="#"
)
...
results_df.loc[
    results_df.Model == icenet_ID_pretrain_ablation, "Model"
] = "IceNet-noCMIP"
...
results_df = results_df.loc(axis=0)[
    pd.IndexSlice[:, :, :, slice(heldout_start, heldout_end)]
]
...
if True:
    ...
    mask = np.load(
        os.path.join(
            config.mask_data_folder, config.active_grid_cell_file_format.format("09")
        )
    )
    ...
    for year_i, year in enumerate(years):
        ...
        for leadtime_i, leadtime in enumerate(extreme_leadtimes):
            icenet_sip = icenet_ensemble_mean_sip_da.sel(
                time=pd.Timestamp(year, 9, 1), leadtime=leadtime
            ).data
            ax = axes[year_i, leadtime_i]

            # ax.imshow(icenet_sip[top:bot, left:right], cmap='Blues_r')
            ax.contourf(
                land_mask[top:bot, left:right],
                levels=[0.5, 1],
                colors=[mpl.cm.gray(123)],
            )

            pred_ice_edge_rgba_arr = arr_to_ice_edge_rgba_arr(
                icenet_sip, 0.5, land_mask, region_mask, pred_ice_edge_rgb
            )
            ...
            ice_edge_rgba_arr = arr_to_ice_edge_rgba_arr(
                groundtruth_sic, 0.15, land_mask, region_mask, true_ice_edge_rgb
            )
            ax.imshow(ice_edge_rgba_arr[top:bot, left:right, :])

            sie_err = results_df.loc[
                "IceNet", icenet_ensemble, leadtime, pd.Timestamp(year, 9, 1)
            ]["SIE error"]
            acc = results_df.loc[
                "IceNet", icenet_ensemble, leadtime, pd.Timestamp(year, 9, 1)
            ]["Binary accuracy"]
            t = ax.text(
                s="Binary acc: {:.1f}%".format(acc),
                x=0.99,
                y=0.09,
                fontsize=5,
            ...
            t.set_bbox(dict(facecolor="white", alpha=0.8, edgecolor="white", pad=0.0))
            t = ax.text(
                s="SIE error: {:+.3f} mil km$^2$".format(sie_err / 1e6),
                x=0.99,
                y=0.02,
                fontsize=5,
                transform=ax.transAxes,
            ...
            if year_i == 0 and leadtime_i == 0:
                proxy = [
                    plt.Line2D([0], [1], color=true_ice_edge_rgb),
                    plt.Line2D([0], [1], color=pred_ice_edge_rgb),
                ]
                ax.legend(
                    proxy,
                    ["Observed ice edge", "Predicted ice edge"],
                    loc="upper left",
                    fontsize=5,
                )
...
if True:
    ...
    ax.set_xlabel("")
    ax.set_xlim([0.5, 6.5])
    ax.set_title("a", x=-0.1, loc="left", fontweight="bold")
    ...
    ax.set_title("b", x=-0.1, loc="left", fontweight="bold")
    ax.set_title("IceNet binary accuracy")
    ax.set_xlabel("")
    ax.set_yticks(np.arange(0.5, 12))
    ...
    ax.set_title("c", x=-0.1, loc="left", fontweight="bold")
    ...
    ax.set_title("d", x=-0.1, loc="left", fontweight="bold")
    ...
    plt.subplots_adjust(left=0.15, top=1 - 0.05, bottom=0.05, right=1 - 0.15)
...
if True:
    ...
    extreme_improvement_df = pd.DataFrame(
        columns=["year", "Lead time (months)", "Model", "improvement"]
    )

    for year in np.arange(2012, 2021):
        icenet_acc = results_df.loc[
            pd.IndexSlice["IceNet", icenet_ensemble, :, "{}-09-01".format(year)],
            "Binary accuracy",
        ]
        seas5_acc = results_df.loc[
            pd.IndexSlice["SEAS5", "NA", :, "{}-09-01".format(year)], "Binary accuracy"
        ]
        lintrend_acc = results_df.loc[
            pd.IndexSlice["Linear trend", "NA", :, "{}-09-01".format(year)],
            "Binary accuracy",
        ]
        icenet_acc.values - lintrend_acc.values
        for i, leadtime in enumerate(np.arange(1, 6 + 1)):
            if leadtime in [2, 3, 4]:
                extreme_improvement_df.loc[len(extreme_improvement_df)] = (
                    year,
                    leadtime,
                    "SEAS5",
                    (icenet_acc.values - seas5_acc.values)[i],
                )
                extreme_improvement_df.loc[len(extreme_improvement_df)] = (
                    year,
                    leadtime,
                    "Linear trend",
                    (icenet_acc.values - lintrend_acc.values)[i],
                )
    ...
    with plt.rc_context(
        ...
        for i, ax in enumerate(g.axes.ravel()):
            ax.set_title(
                axis_labels[i], fontsize=fontsize, fontweight="bold", loc="left", x=-0.0
            )
...
if True:
    ...
    ensemble_mean_nocmip_df = compute_heatmap(
        results_df, "IceNet-noCMIP", "ensemble", metric
    )
    ...
    for seed in icenet_seeds:
        ensemble_dfs.append(compute_heatmap(results_df, "IceNet", seed, metric))
    mean_member_cmip_df = (
        pd.concat(ensemble_dfs).groupby("Calendar month").mean().reindex(month_names)
    )
    ...
    mean_member_nocmip_df = (
        pd.concat(ensemble_dfs_nocmip)
        .groupby("Calendar month")
        .mean()
        .reindex(month_names)
    )
    ...
    if True:
        ...
        sns.heatmap(
            ...
            center=0.0,
            annot=True,
            annot_kws=dict(alpha=0.8, fontsize=6.5),
            fmt="+.1f",
            cbar=False,
            # cbar_ax=cbar_ax,
            cbar_kws=cbar_kws,
        ...
        ax.set_title("a", x=-0.05, loc="left", fontweight="bold")
        ...
        # cbar_kws = dict(label='Difference (%)', extend='both', fraction=0.05, aspect=8)
        cbar_kws = dict(
            label="Binary accuracy change (%)", extend="both", fraction=0.05, aspect=8
        )
        cbar_kws["format"] = FuncFormatter(lambda x, p: format(x, "+.1f"))
        ...
        ax.set_title("b", x=-0.05, loc="left", fontweight="bold")
        ...
        ax.set_title("c", x=-0.05, loc="left", fontweight="bold")
        ...
        # ax.tick_params(axis='y', rotation=0)
        plt.subplots_adjust(left=0.09, right=1 - 0.09, top=1 - 0.2, bottom=0.2)
...
if True:
    ...
    uncertainty_test_df = uncertainty_df.loc[
        pd.IndexSlice[:, slice(test_start, test_end)], :
    ]

    fig, ax = plt.subplots(figsize=(2, 2))
    true = uncertainty_test_df.loc["IceNet"]["Ground truth"] >= 0.15
    prob_icenet = uncertainty_test_df.loc["IceNet"].SIP.values
    prob_icenet[prob_icenet > 1.0] = 1.0
    prob_seas5 = uncertainty_test_df.loc["SEAS5"].SIP.values

    prob_true_icenet, prob_pred_icenet = calibration.calibration_curve(
        true, prob_icenet, n_bins=20
    )
    ax.plot(
        prob_pred_icenet,
        prob_true_icenet,
    ...
    prob_true_seas5, prob_pred_seas5 = calibration.calibration_curve(
        true, prob_seas5, n_bins=20
    )
    ax.plot(
        prob_pred_seas5,
        prob_true_seas5,
...
if True:
    ...
    err_da = xr.ufuncs.logical_and(
        true_sic_da < 0.15, icenet_ensemble_mean_sip_da > 0.5
    ) | xr.ufuncs.logical_and(true_sic_da > 0.15, icenet_ensemble_mean_sip_da < 0.5)
    ...
    for col_i, col_dict in fig2_dict.items():
        ...
        for row_i, row_plot in enumerate(row_list):
            ...
            if row_plot == "SIP":
                ...
                sip_arr = icenet_ensemble_mean_sip_da.sel(
                    time=fc_month, leadtime=leadtime
                ).data
                ...
                ax.contour(
                    ...
                    linestyles="dashed",
                    levels=[0.15],
                    colors=[ice_edge_rgba],
                    linewidths=1.0,
                )

                # Overlay binary errors
                clim = (-1.5, 1.5)
                arr_err = err_da.sel(time=fc_month, leadtime=leadtime).data[
                    top:bot, left:right
                ]
                arr_err[np.isnan(arr_err)] = 0.0
                ...
                arr_err_rgba[
                    idx_arrs[0], idx_arrs[1], 3
                ] = 0  # Alpha = 0 where no error is made
                im_err = ax.imshow(arr_err_rgba)

                if col_i == 0:
                    proxy = [
                        plt.Rectangle((0, 0), 1, 1, fc=ice_edge_rgba),
                    ...
                    ax.legend(
                        proxy, ["Ice edge error region"], loc="upper left", fontsize=5
                    )
                ...
                sip = icenet_ensemble_mean_sip_da.sel(time=fc_month, leadtime=leadtime)[
                    top:bot, left:right
                ].data
                ...
                cmap = mpl.colors.ListedColormap(
                    [confident_sea_rgba, ice_edge_region_rgba, confident_ice_rgba]
                )
                norm = mpl.colors.BoundaryNorm(
                    [0, sip_prime_90, 1 - sip_prime_90, 1 + 1e-6], cmap.N
                )
                ...
                if col_i == ncols - 1:
                    ...
                    formatter = mpl.ticker.FuncFormatter(
                        lambda val, loc: region_list[loc]
                    )
                    plt.colorbar(
                        im_sip,
                        cax,
                        ticks=[sip_prime_90 / 2, 0.5, 1 - sip_prime_90 / 2],
                        format=formatter,
                    )
                ...
                if col_i == 0:
                    ...
                    ax.legend(
                        proxy, ["Observed ice edge"], loc="lower left", fontsize=5
                    )
    ...
    for i, ax in enumerate(axes.ravel()):
        ...
        t.set_bbox(dict(facecolor="white", alpha=0.5, edgecolor="none", pad=0.5))
...
if True:
    ...
    icenet_acc = (
        results_df.loc[pd.IndexSlice["IceNet", icenet_ensemble]]
        .reset_index()
        .set_index(["Calendar month", "Leadtime"])[metric]
    )
    seas5_acc = (
        results_df.loc[pd.IndexSlice["SEAS5", "NA"]]
        .reset_index()
        .set_index(["Calendar month", "Leadtime"])[metric]
    )
    lin_acc = (
        results_df.loc[pd.IndexSlice["Linear trend", "NA"]]
        .reset_index()
        .set_index(["Calendar month", "Leadtime"])[metric]
    )

    icenet_better_than_seas5 = (icenet_acc - seas5_acc) >= 0.0
    icenet_better_than_seas5 = (
        icenet_better_than_seas5.groupby(["Calendar month", "Leadtime"])
        .sum()
        .reset_index()
    ...
    icenet_better_than_lin = (icenet_acc - lin_acc) >= 0.0
    icenet_better_than_lin = (
        icenet_better_than_lin.groupby(["Calendar month", "Leadtime"])
        .sum()
        .reset_index()
        .pivot("Calendar month", "Leadtime", metric)
        .reindex(month_names)
...
if True:
    ...
    for year in set(sie_errors_df.reset_index().year):
        for month, leadtime in zip(sio_months, [4, 3, 2]):
            ...
            sie_errors_df.loc[year, month, "IceNet"] = (
                results_df.loc[
                    "IceNet", icenet_ensemble, leadtime, pd.Timestamp(year, 9, 1)
                ]["SIE error"]
                / 1e6
            ...
            sie_errors_df.loc[year, month, "SEAS5"] = (
                results_df.loc["SEAS5", "NA", leadtime, pd.Timestamp(year, 9, 1)][
                    "SIE error"
                ]
                / 1e6
            )
    ...
    std_abs_err_df = (
        sie_errors_df.groupby(["init_month", "Model"]).std()["abs_sie_err"].to_frame()
    )
    mean_abs_err_df = (
        sie_errors_df.groupby(["init_month", "Model"]).mean()["abs_sie_err"].to_frame()
    )

    std_abs_err_df = std_abs_err_df.reset_index().pivot(
        index="init_month", columns="Model", values="abs_sie_err"
    )
    mean_abs_err_df = mean_abs_err_df.reset_index().pivot(
        index="init_month", columns="Model", values="abs_sie_err"
    )

    mean_abs_err_df = mean_abs_err_df.reindex(["June", "July", "Aug"])[
        ["SEAS5", "SIO", "IceNet"]
    ]
    std_abs_err_df = std_abs_err_df.reindex(["June", "July", "Aug"])[
        ["SEAS5", "SIO", "IceNet"]
    ]
    ...
    # Plot std dev
    mean_abs_err_df.plot(
        kind="bar",
        yerr=std_abs_err_df.values / 2,
        ax=g.ax,
        width=0.8,
        legend=False,
        visible=False,
    ...
    plt.title("a", fontweight="bold", loc="left", x=-0.1)
    plt.tight_layout()
    ...
    for i, ax in enumerate(g.axes.ravel()):
        ax.set_title(axis_labels[i], fontweight="bold", loc="left", x=-0.0)
...
if True:
    ...
    gs1 = gridspec.GridSpecFromSubplotSpec(
        nrows=1, ncols=3, subplot_spec=gs_outer[0], wspace=0.7, hspace=0.5
    )

    gs2 = gridspec.GridSpecFromSubplotSpec(
        nrows=1, ncols=2, subplot_spec=gs_outer[1], wspace=0.4
    )
    ...
    for model in ["SEAS5", "IceNet"]:
        ...
        ax.plot(
            sip_primes,
            sip_bounding_df.loc[model].frac_ice_edge,
            lw=lw,
            color=color,
            label=model,
        )
        ...
        ax.plot(
            sip_primes,
            sip_bounding_df.loc[model].frac_all,
            lw=lw,
            color=color,
            label=model,
        )
        ...
        ax.plot(
            sip_primes,
            sip_bounding_df.loc[model].frac_error,
            lw=lw,
            color=color,
            label=model,
        )
        ...
        if model == "IceNet":
            sip_prime_90 = sip_primes[
                np.argwhere(
                    np.diff(
                        np.sign(np.array(sip_bounding_df.loc[model].frac_ice_edge) - 90)
                    )
                )
            ].ravel()[0]
            frac_all_90 = (
                sip_bounding_df.loc[model]
                .frac_all[
                    np.argwhere(np.diff(np.sign(np.array(sip_primes) - sip_prime_90)))
                ]
                .ravel()[0]
            )
            ...
            gs1_axes[0].annotate(
                "", xytext=(0, 90), xy=(sip_prime_90, 90), arrowprops=arrowprops
            )
            gs1_axes[0].annotate(
                "",
                xytext=(sip_prime_90, 90),
                xy=(sip_prime_90, 0),
                arrowprops=arrowprops,
            )
            gs1_axes[0].text(
                s="90% ice\nedge bound\n$p'={:.03f}$".format(sip_prime_90),
                x=sip_prime_90 + 0.02,
                y=5,
            )
            ...
            gs2_axes[0].annotate(
                "", xytext=(90, 0), xy=(90, frac_all_90), arrowprops=arrowprops
            )
            gs2_axes[0].annotate(
                "", xytext=(90, frac_all_90), xy=(0, frac_all_90), arrowprops=arrowprops
            )
            gs2_axes[0].text(
                s="Ice edge region\ncover = {:.1f}%".format(frac_all_90),
                x=25,
                y=frac_all_90 + 5,
            )
    ...
    for i in [0, 1, 2]:
        ...
        gs1_axes[i].set_ylim([0, 1.0])
        gs1_axes[i].set_xlim([-0.003, 0.503])
        gs1_axes[i].set_xticks(np.arange(0, 0.55, 0.1))
        gs1_axes[i].set_yticks(np.arange(0, 105, 10))
        # gs1_axes[i].xaxis.grid(True, which='minor')
        # gs1_axes[i].yaxis.grid(True, which='minor')
        gs1_axes[i].xaxis.set_minor_locator(mpl.ticker.MultipleLocator(0.05))
...
if True:
    ...
    uncertainty_val_df = uncertainty_df.loc[
        pd.IndexSlice[:, slice(val_start, val_end)], :
    ].reset_index()
    ...
    bins["SEAS5"] = np.linspace(0.0, 1.0, 100)
    # 100 bins betw 0 and 1: ensures symmetry for IceNet bins
    bins["IceNet"] = np.linspace(0.0, 1.0, 101)
    ...
    for i, model in enumerate(["IceNet", "SEAS5"]):
        ...
        l = ax.plot([0, 0.5, 1], [0, 0.5, 0], "k", lw=0.5, label="Perfect reliability")
        ...
        ax.set_xlim([0.0, 1.0])
        if i == 0:
            ax.legend(l, ["Perfect calibration"], loc="best", fontsize=6.0)
    ...
    for i in [1, 2]:
        gs1_axes[i].axvline(x=sip_prime_90, color="k", ls="--", lw=0.5)
        gs1_axes[i].axvline(x=1 - sip_prime_90, color="k", ls="--", lw=0.5)
        gs1_axes[i].annotate(
            "Ice edge\nregion",
...
if True:
    ...
    ice_edge_region_test_df = ice_edge_region_df.loc[
        pd.IndexSlice[:, slice(test_start, test_end)], :
    ]
    ...
    gs1 = gridspec.GridSpecFromSubplotSpec(
        nrows=2, ncols=1, subplot_spec=gs_outer[0], wspace=0.5, hspace=0.5
    )
    ...
    ax.yaxis.set_major_formatter(
        mpl.ticker.FuncFormatter(lambda x, pos: "${:.0f}\\times 10^6$".format(x / 1e6))
    )
    # ax.set_ylim((0, 50))
    ...
    years = sorted(
        list(set(ice_edge_region_test_df.reset_index()["Forecast date"].dt.year))
    )
    years.append(np.min(years) - 1)
    for year in years:
        ax.axvline(
            x=pd.Timestamp(year, 12, 15), color="k", linestyle="--", linewidth=0.5
        )
    ...
    xticks_timestamps = sorted(
        list(set(ice_edge_region_test_df.reset_index()["Forecast date"]))
    )
    ...
    gs2 = gridspec.GridSpecFromSubplotSpec(
        nrows=1, ncols=2, subplot_spec=gs_outer[1], wspace=0.5, hspace=0.5
    )

    line_kws = dict(
        err_style="bars", linewidth=1.0, err_kws=dict(capsize=3.0, barsabove=True)
    )
...
if True:
    ...
    ice_edge_region_heatmap = (
        ice_edge_region_df.reset_index()[["Leadtime", "Calendar month", "Coverage"]]
        .groupby(["Calendar month", "Leadtime"])
        .mean()
        .reset_index()
        .pivot("Calendar month", "Leadtime", "Coverage")
        .reindex(month_names)
    )

    area_heatmap = (
        ice_edge_region_df.reset_index()[["Leadtime", "Calendar month", "Area"]]
        .groupby(["Calendar month", "Leadtime"])
        .mean()
        .reset_index()
        .pivot("Calendar month", "Leadtime", "Area")
        .reindex(month_names)
    )
    ...
    ax = axes[0]
    sns.heatmap(
        data=ice_edge_region_heatmap,
        annot=False,
        # fmt=df1_fmt,
        cbar_kws=dict(
            label="Binary error coverage (%)", extend="both", fraction=0.05, aspect=8
        ),
        # cmap=cmap1,
        # cbar_ax=cbar_ax1,
        ax=ax,
    )
    ax.set_title("a", fontweight="bold", x=-0.15, loc="left")
    ax.set_ylabel("Calendar month")
    ax.set_xlabel("Lead time (months)")
    ...
    sns.heatmap(
        ...
        annot=False,
        cbar_kws=dict(
            label="Ice edge region area (km$^2$)",
            extend="both",
            fraction=0.05,
            aspect=8,
            format=mpl.ticker.FuncFormatter(
                lambda x, pos: "${:.0f}\\times 10^6$".format(x / 1e6)
            ),
        ),
        # fmt=fmt,
        # cbar_kws=cbar_kws,
    ...
    ax.set_title("b", fontweight="bold", x=-0.15, loc="left")
...
if True:
    ...
    for month in table_month_names:
        ...
        mean_results_df_verbose = mean_results_df_verbose[
            (mean_results_df_verbose.Variable != "cos(month)")
        ...
        for varname, leadtime in zip(short_varnames, leadtimes):
            verbose_varname = utils.make_varname_verbose(
                varname, leadtime, fc_month_idx
            )
            verbose_varnames.append(verbose_varname)
        ...
        for i, leadtime in enumerate(leadtimes_to_plot):
            ...
            leadtime_sorted_top5_df = (
                ...
                .drop("Leadtime", axis=1)
                .set_index("Variable")
                .sort_values("Accuracy drop (%)")[0:5]
            )
            ...
            for drop_i in range(len(acc_drops)):
                ...
                string += " ({:.2f}%) ".format(
                    leadtime_sorted_top5_df.values.ravel()[drop_i]
                )
            ...
            table_df[
                "{} intitialisation ({} month leadtime)".format(
                    initialisation_month, leadtime
                )
            ] = labels.ravel()
