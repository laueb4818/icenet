...
dataloader_ID_folder = os.path.join(
    config.networks_folder, dataloader_ID, "obs_train_val_data"
)
...
def convert_batch(tf_path, data):
    ...
    with tf.io.TFRecordWriter(tf_path) as writer:
        ...
        for i in range(data[0].shape[0]):
            ...
            logging.info(
                "x shape {}, y shape {}, w shape {}".format(x.shape, y.shape, w.shape)
            )

            record_data = tf.train.Example(
                features=tf.train.Features(
                    feature={
                        ...
                        "w": tf.train.Feature(
                            float_list=tf.train.FloatList(value=w.reshape(-1))
                        ),
                    }
                )
            ).SerializeToString()
...
if __name__ == "__main__":
    ...
    for data in [
        ...
        sample_weight_data = os.path.join(
            dataloader_ID_folder, "numpy", "{}.npy".format(data[2])
        )
