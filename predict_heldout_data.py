...
heldout_end = pd.Timestamp("2021-07-01")
...
for model in models:
    ...
    if model == "IceNet":
        dims = ("time", "yc", "xc", "leadtime", "ice_class")
...
for model in models:
    ...
    for start_date in tqdm(all_start_dates, position=0, leave=False, file=sys.stdout):
        ...
        if model == "IceNet":
            ...
            pred = np.array(
                [
                    load_model(path, compile=False).predict(X)[0]
                    for path in network_fpaths
                ]
            )
        ...
        for i, (target_date, leadtime) in enumerate(zip(target_dates, leadtimes)):
            ...
            if model == "IceNet":
                if target_date in all_target_dates:
                    model_forecast_dict[model].loc[target_date, :, :, leadtime] = pred[
                        ..., i
                    ]
    ...
    forecast_fpath = os.path.join(
        forecast_folders_dict[model], f"{model_str}_forecasts.nc".format(model_str)
    )
    ...
    if model == "IceNet":
        ...
        icenet_ensemble_mean_da = model_forecast_dict[model]
        icenet_ensemble_mean_sip_da = icenet_ensemble_mean_da.sel(
