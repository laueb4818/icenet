...
def numpy_to_xarray(arr, init_dates, target_dates, n_forecast_months):
    ...
    ds = xr.DataArray(data=np.zeros(shape, dtype=int), coords=coords, dims=dims)
    ...
    for init_date_idx, init_date in enumerate(init_dates):
        ...
        for leadtime_idx, (target_date, leadtime) in enumerate(
            zip(forecast_target_dates, leadtimes)
        ):
            if target_date in target_dates:
                ds.loc[target_date, :, :, leadtime] = arr[
                    init_date_idx, ..., leadtime_idx
                ]
...
network_paths = [
    os.path.join(network_h5_files_folder, filename) for filename in filenames
]
...
true_ds = numpy_to_xarray(true_outputs, init_dates, target_dates, n_forecast_months)
...
tic = time.time()
icenet_preds_unpermuted = run_icenet_on_numpy(unpermuted_inputs, ensemble, batch_size)
...
for seed in tqdm(seeds):
    ...
    for var_i, varname in enumerate(tqdm(all_ordered_variable_names, leave=False)):
        ...
        icenet_preds_permuted = run_icenet_on_numpy(
            permuted_inputs, ensemble, batch_size
        )
        ...
        for target_date in target_dates:
            for leadtime in leadtimes:
                pap_results_df.loc[
                    seed, leadtime, target_date, varname
