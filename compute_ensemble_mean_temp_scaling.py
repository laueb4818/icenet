...
print(
    "Finding optimal T for each leadtime using Brent optimisation... ",
    end="",
    flush=True,
)
...
for leadtime in icenet_forecast_da.leadtime.values:
    ...
    T_ens_opt = scipy.optimize.minimize_scalar(
        func,
        args=(ensemble_mean_logits_leadtime, leadtime),
        bracket=(0.1, 2.0),
        method="brent",
        tol=1e-6,
    )
...
ensemble_mean_forecast_da = xr.open_dataarray(heldout_forecast_fpath).sel(
    seed="ensemble"
)
...
mask_arr = np.tile(
    np.stack(
        [
            np.load(mask_fpath_format.format("{:02d}".format(pd.Timestamp(date).month)))
            for date in icenet_forecast_tempscaled_da.time.values
        ]
    )[..., np.newaxis, np.newaxis],
    [1, 1, 1, 6, 3],
)
icenet_forecast_tempscaled_da = icenet_forecast_tempscaled_da.where(mask_arr, 0.0)
...
if "ensemble_tempscaled" in full_icenet_forecast_da.seed.values:
    ...
    full_icenet_forecast_da = full_icenet_forecast_da.where(
...
full_icenet_forecast_da = xr.concat(
    [full_icenet_forecast_da, icenet_forecast_tempscaled_da], dim="seed"
)
...
fpath = os.path.join(
    ...
    dataloader_ID,
    architecture_ID,
    "icenet_sip_forecasts_tempscaled.nc",
)
icenet_ensemble_mean_sip_da = icenet_forecast_tempscaled_da.sel(
    ice_class=["marginal_ice", "full_ice"]
).sum("ice_class")
