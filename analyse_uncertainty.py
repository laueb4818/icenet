...
### SEAS5 forecasts
fpath = os.path.join(config.forecast_data_folder, "seas5", "seas5_sip_forecasts.nc")
...
mask_fpath_format = os.path.join(
    config.mask_data_folder, config.active_grid_cell_file_format
)
...
def get_ice_edge_vector(date):
    ...
    ice_edge_arr = arr_to_ice_edge_arr(arr, 0.15, land_mask, region_mask)
    mask = np.load(mask_fpath_format.format("{:02d}".format(date.month)))
...
print(
    "Building up dataset of vector predictions for each forecast... ",
    end="",
    flush=True,
)
...
# True negative
uncertainty_df.loc[
    (uncertainty_df["Ground truth"] == 0) & (uncertainty_df["SIP"] <= 0.5), "Error?"
] = False
# True positive
uncertainty_df.loc[
    (uncertainty_df["Ground truth"] == 1) & (uncertainty_df["SIP"] >= 0.5), "Error?"
] = False
...
print(
    "Computing weight values for validation SIP error rate histogram... ",
    end="",
    flush=True,
)
...
bins["SEAS5"] = np.linspace(0.0, 1.0, 100)  # Note: bin edges
# 100 bins betw 0 and 1: ensures symmetry for IceNet bins
bins["IceNet"] = np.linspace(0.0, 1.0, 101)

uncertainty_df["Histogram weight"] = 0.0

# Validation data only
uncertainty_val_df = (
    uncertainty_df.reset_index()
    .set_index("Forecast date")
    .sort_index()
    .loc[slice(val_start, val_end)]
)

uncertainty_val_df = uncertainty_val_df.reset_index().set_index("Model").sort_index()
...
for model in ["IceNet", "SEAS5"]:
    ...
    bin_counts = plt.hist(uncertainty_val_df.loc[model].SIP, bins=bins[model])[0]
    ...
    uncertainty_df.loc[model, "Histogram weight"] = weights[
        pd.cut(
            uncertainty_df.loc[model, "SIP"],
            bins=bins[model],
...
sip_bounding_df = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        (["IceNet", "SEAS5"], sip_primes), names=["Model", "p'"]
    ),
...
for model in ["SEAS5", "IceNet"]:
    ...
    sip_error = (
        uncertainty_val_df.reset_index()
        .set_index(["Model", "Error?"])
        .sort_index()
        .loc[model, True]
        .SIP.values.copy()
    )
    ...
    frac_ice_edge = 1 - np.cumsum(
        np.histogram(sip_ice_edge, bins=sip_primes)[0] / norm_ice_edge
    )
    # Account for first bin edge, which integrates to 1
    ...
    if model == "IceNet":
        sip_prime_95 = sip_primes[
            np.argwhere(
                np.diff(
                    np.sign(np.array(sip_bounding_df.loc[model, "frac_ice_edge"]) - 95)
                )
            )
        ].ravel()[0]
        ...
        frac_all_95 = frac_all[
            np.argwhere(np.diff(np.sign(np.array(sip_primes) - sip_prime_95)))
        ].ravel()[0]
        frac_all_90 = frac_all[
            np.argwhere(np.diff(np.sign(np.array(sip_primes) - sip_prime_90)))
        ].ravel()[0]

        print(model)
        print(
            "p' for 95% ice edge bounding: {:.4f}. Fraction of preds in "
            "ice edge regions: {:.1f}".format(sip_prime_95, frac_all_95)
        )
        print(
            "p' for 90% ice edge bounding: {:.4f}. Fraction of preds in "
...
area_df = (
    uncertainty_df.loc["IceNet"]
    .groupby(["Leadtime", "Forecast date"])
    .SIP.apply(lambda p: np.sum((p > sip_prime_90) & (p < 1 - sip_prime_90)) * 25**2)
    .to_frame()
    .reset_index()
)
...
ice_edge_region_df = (
    ...
    .groupby(["Leadtime", "Forecast date"])
    .SIP.apply(
        lambda p: 100 * np.sum((p > sip_prime_90) & (p < 1 - sip_prime_90)) / len(p)
    )
    .to_frame()
    .reset_index()
)
...
forecast_month_names = month_names[
    ice_edge_region_df["Forecast date"].dt.month.values - 1
]
