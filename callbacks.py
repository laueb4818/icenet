...
class IceNetPreTrainingEvaluator(tf.keras.callbacks.Callback):
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        if (batch == 0 and self.sample_at_zero) or (
            batch + 1
        ) % self.validation_frequency == 0:
            val_logs = self.model.evaluate(
                self.val_dataloader, verbose=0, return_dict=True
            )
        ...
        if (batch == 0 and self.sample_at_zero) or (
            ...
            if self.log_weights:
                ...
                for layer in self.model.layers:
                    ...
                    if len(weights) == 1:
                        metrics[
                            "parameters/" + layer.name + ".weights"
                        ] = wandb.Histogram(weights[0])
                    elif len(weights) == 2:
                        metrics[
                            "parameters/" + layer.name + ".weights"
                        ] = wandb.Histogram(weights[0])
            ...
            if save:
                print(
                    "\n{} improved from {:.3f} to {:.3f}. Saving model to {}.\n".format(
                        self.monitor, self.best, logs[self.monitor], self.model_path
                    )
                )
...
class IceNetCaseStudyCallback(tf.keras.callbacks.Callback):
    def __init__(self, dataloader, network_folder, seed, include_train=False):
        ...
        if include_train:
            (
                ...
                self.y_train,
                self.mask_train,
            ) = dataloader.get_month_of_input_output_data(self.train_date)

            self.class_train = []
            self.class_train_vector = []
            for forecast_month_idx in range(self.n_forecast_months):
                class_train = np.argmax(
                    self.y_train[0, :, :, 1:, forecast_month_idx], axis=-1
                )
                self.class_train.append(class_train)
                self.class_train_vector.append(
                    class_train[self.mask_train[forecast_month_idx, :]]
                )
        ...
        (
            ...
            self.y_val,
            self.mask_val,
        ) = dataloader.get_month_of_input_output_data(self.val_date)
        ...
        for forecast_month_idx in range(self.n_forecast_months):
            ...
            self.class_val_vector.append(
                class_val[self.mask_val[forecast_month_idx, :]]
            )
        ...
        self.folder = os.path.join(
            network_folder, "{}_training_case_studies".format(seed)
        )
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.include_train:
            ...
            for forecast_month_idx in range(self.n_forecast_months):
                class_pred_train = np.argmax(
                    y_pred_train[0, :, :, :, forecast_month_idx], axis=-1
                )
                class_pred_train[~self.mask_train[forecast_month_idx, :]] = 0
                self.class_pred_train.append(class_pred_train)
                self.class_pred_train_vector.append(
                    class_pred_train[self.mask_train[forecast_month_idx, :]]
                )
            ...
            for forecast_month_idx in range(self.n_forecast_months):
                self.train_acc.append(
                    100
                    * accuracy_score(
                        self.class_train_vector[forecast_month_idx],
        ...
        for forecast_month_idx in range(self.n_forecast_months):
            class_pred_val = np.argmax(
                y_pred_val[0, :, :, :, forecast_month_idx], axis=-1
            )
            class_pred_val[~self.mask_val[forecast_month_idx, :]] = 0
            self.class_pred_val.append(class_pred_val)
            self.class_pred_val_vector.append(
                class_pred_val[self.mask_val[forecast_month_idx, :]]
            )
        ...
        for forecast_month_idx in range(self.n_forecast_months):
            self.val_acc.append(
                100
                * accuracy_score(
                    self.class_val_vector[forecast_month_idx],
        ...
        fig, axes = plt.subplots(
            nrows=nrows, ncols=self.n_forecast_months, figsize=figsize
        )

        for forecast_month_idx in range(self.n_forecast_months):
            if self.include_train:
                axes[0, forecast_month_idx].imshow(
                    self.class_train[forecast_month_idx], cmap="Blues_r"
                )
                axes[0, forecast_month_idx].set_title(
                    (
                        self.train_date + relativedelta(months=forecast_month_idx)
                    ).strftime("%Y_%m"),
                    fontsize=30,
                )
                axes[1, forecast_month_idx].imshow(
                    self.class_pred_train[forecast_month_idx], cmap="Blues_r"
                )
                axes[1, forecast_month_idx].set_xlabel(
                    "Acc: {:.2f}".format(self.train_acc[forecast_month_idx]),
                    fontsize=30,
                )
                ...
                axes[2, forecast_month_idx].imshow(
                    self.class_val[forecast_month_idx], cmap="Blues_r"
                )
                axes[2, forecast_month_idx].set_title(
                    (self.val_date + relativedelta(months=forecast_month_idx)).strftime(
                        "%Y_%m"
                    ),
                    fontsize=30,
                )
                axes[3, forecast_month_idx].imshow(
                    self.class_pred_val[forecast_month_idx], cmap="Blues_r"
                )
                axes[3, forecast_month_idx].set_xlabel(
                    "Acc: {:.2f}".format(self.val_acc[forecast_month_idx]), fontsize=30
                )
        ...
        for ax in axes.ravel():
            ax.contourf(
                np.load(self.dataloader.variable_paths["land"]),
                levels=[0, 1],
                colors="gray",
                extend="neither",
            )
